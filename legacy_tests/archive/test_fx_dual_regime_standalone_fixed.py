...
def main():
    ...
    print(
        f"  FX range: {recent_df['Avg FX Rate'].min():.2f} - {recent_df['Avg FX Rate'].max():.2f} LKR/USD"
    )
    ...
    try:
        ...
        for name, path in recent_scenarios.items():
            result = module_recent.assess_revenue_debt_mismatch(
                40_000_000, 2_400_000_000, 305.0, path
            )
    ...
    print(
        f"  Coverage: {(historical_df['Year-Month'].max() - historical_df['Year-Month'].min()).days / 365.25:.1f} years"
    )
    print(
        f"  FX range: {historical_df['Avg FX Rate'].min():.2f} - {historical_df['Avg FX Rate'].max():.2f} LKR/USD"
    )
    ...
    try:
        ...
        for name, path in historical_scenarios.items():
            result = module_historical.assess_revenue_debt_mismatch(
                40_000_000, 2_400_000_000, 305.0, path
            )
