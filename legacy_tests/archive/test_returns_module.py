...
for i in range(len(ds_list)):
    y = i + 1
    print(
        f"{y:4d} | {cfads_gross[i]/1e6:10.2f}M | {cfads_list[i]/1e6:10.2f}M | {ds_list[i]/1e6:10.2f}M | {tax_list[i]/1e6:10.2f}M"
    )

print("\n--- SUMMARY ---")
print(
    f"Leverage: {equity['equity_investment']/(project['project_npv']+equity['equity_investment']):.2%}"
)
print("=" * 80)
