"""DutchBay EPC Financial Contracts and Data Structures
Sprint 9: Tornado Analysis & Sensitivity Modeling
"""
...
# ============================================================================
# TORNADO ANALYSIS & SENSITIVITY MODELS (Core Sprint 9 Deliverables)
# ============================================================================

class TornadoResult(BaseModel):
    """Result of a tornado analysis for a single variable."""
    variable: str
    base_irr: float
    low_irr: float
    high_irr: float
    ...
    def impact_pct(self) -> float:
        """Percentage impact: (high - low) / abs(base) * 100"""
        if self.base_irr == 0:
            return 0.0
    ...
    def impact_abs(self) -> float:
        """Absolute impact: high - low"""
        return self.high_irr - self.low_irr


class ParameterRangeConfig(BaseModel):
    """Configuration for parameter range in tornado analysis."""
    variable_name: str
    base_value: float
    low_pct: float
    high_pct: float
    ...
    def validate_low_pct(cls, v):
        """Ensure low_pct is negative and within bounds."""
        if v >= 0:
    ...
    def validate_high_pct(cls, v):
        """Ensure high_pct is positive and within bounds."""
        if v <= 0:
    ...
    def validate_steps(cls, v):
        """Ensure steps is within acceptable range."""
        if v < 3:
    ...
    def compute_and_validate_range(self):
        """Compute low_value and high_value after all field validation."""
        # Validate range relationship
        if self.high_pct <= abs(self.low_pct):
            raise ValueError(
                f"high_pct ({self.high_pct}%) must exceed abs(low_pct) ({abs(self.low_pct)}%)"
            )
    ...
    def generate_values(self) -> List[float]:
        """Generate linspace of values from low to high using steps."""
...
class TornadoMetrics(BaseModel):
    """Aggregated metrics from tornado analysis."""
    ...
    def from_results(cls, results: List[TornadoResult]) -> 'TornadoMetrics':
        """Calculate metrics from tornado results."""
