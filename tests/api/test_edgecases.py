...
def _make_edge_case_config(case_name: str) -> dict:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # V14 FX COMPLIANCE (R22): All configs must define fx mapping
    # ─────────────────────────────────────────────────────────────────────────
    # Edge-case tests probe specific logic (tax, DSCR, etc.), not FX behavior.
    # To avoid false negatives from schema_guard_fx_validation, ensure a
    # minimal valid FX block exists in all edge cases.
    #
    # Use neutral values (no depreciation) so edge tests fail ONLY on their
    # intended dimension, not on missing FX infrastructure.
    #
    # Go with the Flow principle R22: All currency flows via canonical FX map.
    # ─────────────────────────────────────────────────────────────────────────
    if "fx" not in base:
        base["fx"] = {
            "start_lkr_per_usd": 375.0,  # Realistic base rate (Dec 2024)
            "annual_depr": 0.0,  # Neutral for edge tests
        }
...
def test_zero_tax_rate_edge_case_passes() -> None:
    ...
    logger.debug(f"Config fx block: {config.get('fx', {})}")
