...
def test_qra_contingency_is_base_times_sigma_times_z() -> None:
    r = resolve_contingency(
        100_000_000.0,
        {"contingency": {"method": "qra", "base_cost_uncertainty_pct": 10.0, "confidence_level": 0.80}},
    )
    assert isinstance(r, ContingencyResult)
    assert r.method == "qra"
    assert r.contingency_usd == pytest.approx(100_000_000.0 * 0.10 * _Z80, rel=1e-3)  # ~8.42M
...
def test_higher_confidence_raises_contingency() -> None:
    ...
    p80 = resolve_contingency(100e6, {"contingency": {**cfg["contingency"], "confidence_level": 0.80}})
    p90 = resolve_contingency(100e6, {"contingency": {**cfg["contingency"], "confidence_level": 0.90}})
...
def test_qra_requires_uncertainty_and_valid_confidence() -> None:
    ...
    with pytest.raises(ValueError, match="confidence_level"):
        resolve_contingency(
            100e6,
            {"contingency": {"method": "qra", "base_cost_uncertainty_pct": 10.0, "confidence_level": 1.0}},
        )
...
def test_capex_derivation_uses_qra_contingency() -> None:
    ...
    cfg = {
        "capex": {
            ...
            "contingency": {"method": "qra", "base_cost_uncertainty_pct": 12.0, "confidence_level": 0.80},
            "breakdown": {
...
def test_capex_derivation_default_is_breakdown_sum() -> None:
    ...
    cfg = {
        "capex": {
            ...
            "breakdown": {"wtg_usd": 110e6, "bop_usd": 30e6, "grid_usd": 12e6, "contingency_usd": 2e6},
        }
    }
