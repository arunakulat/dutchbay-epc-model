...
def test_bad_params_are_caught_by_schema_guard(edge_case: str) -> None:
    ...
    assert error_msg and ("required" in error_msg.lower() or "invalid" in error_msg.lower()), (
...
@pytest.mark.skip(reason="Range validation (capex > 0, etc.) is in sensitivity layer, not schema_guard")
@pytest.mark.parametrize("edge_case", [
