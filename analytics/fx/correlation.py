"""FX Analytics Module - Migrated to analytics/fx/.

Migrated from: root-level script
...
class FXCorrelationModuleCorrected:
    ...
    def fx_sensitivity_analysis(self, fx_range_pct: List[float] = None) -> Dict:
        """
        ...
        for pct_change in fx_range_pct:
            ...
            dscr_change_pct = (
                ((dscr - base_dscr) / base_dscr * 100) if base_dscr != 0 else 0
            )
    ...
    def paydown_schedule_analysis(
        self, paydown_scenarios: Dict[str, float] = None, years: int = 15
    ) -> Dict:
        """
        ...
        for scenario_name, paydown_pct in paydown_scenarios.items():
            ...
            for year in range(1, years + 1):
                ...
                usd_ratio = (
                    usd_debt_remaining / total_debt_usd if total_debt_usd > 0 else 0
                )
    ...
    def stress_test_paydown_scenarios(
        self, paydown_scenarios: Dict[str, float] = None, fx_shocks: List[float] = None
    ) -> Dict:
        """
        ...
        for scenario_name, paydown_pct in paydown_scenarios.items():
            ...
            for fx_rate in fx_shocks:
                dscr = self.calculate_dscr(
                    self.annual_lkr_revenue, self.annual_lkr_debt, usd_debt, fx_rate
                )
    ...
    def monte_carlo_paydown_optimization(
        self, fx_scenarios: int = 1000, years_ahead: int = 15, target_dscr: float = 1.25
    ) -> Dict:
        """
            ...
            for _ in range(fx_scenarios):
                ...
                for year, fx_rate in enumerate(fx_path, 1):
                    usd_debt = self.annual_usd_debt_origination * (
                        1 - paydown_speed * (year / years_ahead)
                    )
