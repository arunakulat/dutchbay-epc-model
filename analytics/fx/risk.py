...
class TailRiskAnalyzer:
    ...
    def percentile_analysis(
        self, returns: np.ndarray, percentiles: List[int] = [10, 25, 50, 75, 90]
    ) -> Dict[str, float]:
        """
    ...
    def downside_risk(
        self, returns: np.ndarray, target_return: float = 0.0
    ) -> Dict[str, float]:
        """
        ...
        sortino = (
            (mean_return - target_return) / downside_std if downside_std > 0 else np.inf
        )
    ...
    def covenant_breach_probability(
        self, metric_series: List[np.ndarray], min_thresholds: Dict[str, float]
    ) -> Dict[str, Any]:
        """
    ...
    def tail_risk_report(
        ...
        report = {
            "equity_irr": {
                ...
                "probability_below_hurdle": (equity_irr < target_equity_irr).sum()
                / len(equity_irr),
            },
            "project_irr": {
