...
class MonteCarloEngine:
    ...
    def __init__(self, config: DictConfig, n_iterations: int = 1000) -> None:
        ...
        if configured_discount is not None:
            ...
            self.logger.info("No discount_rate_pct in config; using default 8.0%")
            discount_rate_pct = 8.0
    ...
    def simulate_iteration(self) -> dict[str, Any]:
        ...
        revenue_factor = np.random.normal(
            loc=1.0, scale=self.mc_config.revenue_std_pct / 100.0
        )
        cost_factor = np.random.normal(
            loc=1.0, scale=self.mc_config.cost_std_pct / 100.0
        )
