...
def plot_tornado_chart(
    ...
    df = pd.DataFrame([{
        "Parameter": t.label, "Low": t.low_metric,
        "Base": t.base_metric, "High": t.high_metric
    ...
    for i, (b, l, h) in enumerate(zip(base, low, high)):
        ax.barh(i, b - l, left=l, color="#d35400", height=0.5, label="Low" if i == 0 else "")
        ax.barh(i, h - b, left=b, color="#2980b9", height=0.5, label="High" if i == 0 else "")
...
def plot_spider_chart(
    ...
    df = multi_metric_suite_to_dataframe(suite).sort_values(f"Impact_{metrics[0]}", ascending=False)[:top_n]
