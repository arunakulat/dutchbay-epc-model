- Close the complete internally visible compatibility-assertion graph directly in the public,
  ...
  can be masked by a containing `EvaluationRequest`. D3B v1 explicitly fails closed when multiple
  physical assets reuse one technology binding; supporting that valid D3A topology requires a later
  allocation design.
