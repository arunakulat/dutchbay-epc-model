...
class ERA5Fetcher:
    ...
    def __init__(
        self, cache_dir: str = "inputs/wind_data", config_path: Optional[str] = None
    ) -> None:
    ...
    def _load_config(self, config_path: Optional[str]) -> None:
        ...
        try:
            ...
            self.reference_height = self.config["wind_shear"]["reference_heights"][
                1
            ]  # 100m
        ...
        logger.debug(
            f"Config loaded: area_buffer={self.area_buffer}, "
    ...
    def download_wind_data(
        ...
        cache_file = self.cache_dir / (
            f"era5_{location['name'].lower()}_" f"{start_date}_to_{end_date}.csv"
        )
    ...
    def _download_from_cds(
        self, location: Dict[str, Any], start_date: str, end_date: str
    ) -> Path:
        ...
        Raises:
            RuntimeError: If CDS API request fails.
        """
        nc_file = self.cache_dir / (f"era5_{location['name'].lower()}_raw.nc")
    ...
    def extrapolate_to_hub_height(
        self, df: pd.DataFrame, hub_height: float = 150.0
    ) -> pd.DataFrame:
        ...
        df[col_name] = (
            df["ws_100m"] * (hub_height / self.reference_height) ** df["alpha"]
        )

        logger.info(
            f"Extrapolated to {hub_height}m: " f"mean = {df[col_name].mean():.2f} m/s"
        )

        return df

    def _save_metadata(
        self, location: Dict[str, Any], start_date: str, end_date: str, cache_file: Path
    ) -> None:
