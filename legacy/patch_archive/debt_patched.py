...
def calculate_construction_drawdowns(
    ...
    if cumulative_drawn > total_debt:
        logger.warning(
            f"Total drawn {cumulative_drawn:.2f} exceeds total debt {total_debt:.2f}"
        )
...
def calculate_idc(
    debt_drawn_schedule: List[float], interest_rate: float, construction_periods: int
...
def _sculpted_schedule(
    ...
    for _ in range(amort_years):
        cf = (
            cfads[year_index]
            if year_index < len(cfads)
            else (cfads[-1] if cfads else 0.0)
        )
...
def apply_debt_layer(
    ...
    logger.info(
        f"V14 Debt Planning: {construction_periods}-year construction, {tenor}-year tenor"
    )
    ...
    for tranche_name, tranche in tranches.items():
        ...
        drawn = calculate_construction_drawdowns(
            tranche.principal, construction_schedule, drawdown_pct
        )

        # Calculate IDC
        idc_per_period, total_idc_cap = calculate_idc(
            drawn, tranche.rate, construction_periods
        )
        ...
        logger.info(
            f"  {tranche_name}: Principal ${tranche.principal:.2f}M (IDC: ${total_idc_cap:.2f}M)"
        )
    ...
    logger.info(
        f"V14 Results: Min DSCR={dscr_min:.2f}, Total IDC=${sum(total_idc_by_tranche.values()):.2f}M"
    )
