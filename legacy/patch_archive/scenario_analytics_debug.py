...
class ScenarioAnalytics:
    ...
    def __init__(
        ...
        self.chart_gen = ChartGenerator(
            output_dir=os.path.join(os.path.dirname(output_path), "charts")
        )
        ...
        try:
            ...
            for i, row in enumerate(annual_rows):
                ts_row = {
                    ...
                    "debt_outstanding": (
                        debt_series[i] if i < len(debt_series) else None
                    ),
                }
            ...
            self.failed_scenarios.append({"scenario": scenario_name, "error": str(e)})
            return None
    ...
    def run_batch_analysis(self) -> pd.DataFrame:
        ...
        print(f"\n{'='*60}")
        print(
            f"Batch analysis complete: {len(self.scenario_results)} succeeded, {len(self.failed_scenarios)} failed"
        )
        print(f"{'='*60}\n")
        ...
        if "final_cfads" in summary_df.columns:
            ...
            if not neg_cfads.empty:
                ...
                for _, row in neg_cfads.iterrows():
                    print(
                        f"  - {row['scenario']}: Final CFADS = ${row['final_cfads']:,.0f}"
                    )
            else:
    ...
    def generate_charts(
        self, summary_df: pd.DataFrame, timeseries_df: pd.DataFrame
    ) -> Dict[str, str]:
        ...
        if dscr_data:
            chart_paths["dscr"] = self.chart_gen.plot_dscr_comparison(
                dscr_data, threshold=self.dscr_threshold
            )
        ...
        if "npv" in summary_df.columns and not summary_df["npv"].isna().all():
            chart_paths["npv"] = self.chart_gen.plot_kpi_comparison(
                summary_df[["scenario", "npv"]].dropna(), "npv", "npv_comparison.png"
            )
        ...
        if "irr" in summary_df.columns and not summary_df["irr"].isna().all():
            ...
            chart_paths["irr"] = self.chart_gen.plot_kpi_comparison(
                irr_df[["scenario", "irr_pct"]], "irr_pct", "irr_comparison.png"
            )
            ...
            for chart_name, chart_path in chart_paths.items():
                if os.path.exists(chart_path):
                    self.excel_exporter.add_chart_image(
                        "Charts", chart_path, f"B{row_offset}"
                    )
    ...
    def create_wide_format(self, timeseries_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        for scenario in scenarios:
            scenario_data = timeseries_df[
                timeseries_df["scenario"] == scenario
            ].sort_values("year")
    ...
    Examples:
      ...
      # With custom discount rate and DSCR threshold
      ...
      python analytics/scenario_analytics.py --dir ./scenarios --output ./exports/report.xlsx --no-charts
            """,
        )
        ...
        parser.add_argument(
            ...
            "-o",
            required=True,
            help="Output Excel file path (e.g., ./exports/report.xlsx)",
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=1.25,
            help="DSCR threshold for QC checks (default: 1.25)",
        )
        ...
        parser.add_argument(
            "--no-csv", action="store_true", help="Skip CSV backup exports"
        )
        ...
        analytics.run(generate_charts=not args.no_charts, export_csv=not args.no_csv)
