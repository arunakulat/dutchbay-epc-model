...
from dutchbay_v13.finance.metrics import (calculate_llcr, calculate_plcr,
                                          check_llcr_covenant,
                                          check_plcr_covenant,
                                          compute_dscr_series, summarize_dscr)
...
class TestCoverageRatios(unittest.TestCase):
    def test_llcr_basic(self):
        ...
        if RICH:
            console.print(
                Panel(
                    f"LLCR: min={llcr['llcr_min']:.3f}x, avg={llcr['llcr_avg']:.3f}x",
                    title="LLCR Result",
                    style="green",
                )
            )
    ...
    def test_plcr_basic(self):
        ...
        if RICH:
            console.print(
                Panel(
                    f"PLCR: min={plcr['plcr_min']:.3f}x, avg={plcr['plcr_avg']:.3f}x",
                    title="PLCR Result",
                    style="green",
                )
            )
    ...
    def test_dscr_full(self):
        ...
        if RICH:
            console.print(
                Panel(
                    f"DSCR: min={dscr_summary['dscr_min']:.2f}x, avg={dscr_summary['dscr_avg']:.2f}x",
                    title="DSCR Summary",
                    style="cyan",
                )
            )
    ...
    def test_dash_summary_and_relationships(self):
        ...
        if RICH:
            ...
            console.print(
                Panel(
                    f"PLCR > LLCR: {plcr['plcr_min']:.2f}x > {llcr['llcr_min']:.2f}x",
                    style=style,
                )
            )
...
if __name__ == "__main__":
    ...
    if RICH and result.wasSuccessful():
        console.print(
            Panel("All coverage and covenant tests PASSED ✓", style="bold green")
        )
    elif not result.wasSuccessful():
        err_count = len(result.failures) + len(result.errors)
        style = "red" if err_count else "yellow"
        console.print(
            Panel(f"Tests completed with {err_count} failures/errors.", style=style)
        )
