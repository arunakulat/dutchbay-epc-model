...
def _calculate_annuity_payment(
    ...
    factor = (
        annual_rate * ((1 + annual_rate) ** years) / (((1 + annual_rate) ** years) - 1)
    )
...
def apply_debt_layer(
    financing_params: Dict[str, Any], annual_rows: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
    ...
    rate_usd = _as_float(
        rates.get("usd_nominal") or rates.get("usd_commercial_min"), 0.075
    )
    ...
    capex_usd = _as_float(
        _get(financing_params, ["__capex_usd__"], None)
    )  # Placeholder
    ...
    int_lkr, princ_lkr, bal_lkr = _build_debt_schedule(
        lkr_debt, rate_lkr, tenor_years, interest_only_years
    )
    int_dfi, princ_dfi, bal_dfi = _build_debt_schedule(
        dfi_debt_lkr, rate_dfi, tenor_years, interest_only_years
    )
    int_usd, princ_usd, bal_usd = _build_debt_schedule(
        usd_debt_lkr, rate_usd, tenor_years, interest_only_years
    )
    ...
    for year in range(tenor_years, len(cfads_list)):
        ...
        dscr_series.append(
            _calculate_dscr(cfads_list[year], 0.0) if cfads_list[year] > 0 else None
        )

    # Summary metrics
    dscr_min = min([d for d in dscr_series if d is not None], default=0.0)
    dscr_avg = (
        sum([d for d in dscr_series if d is not None])
        / len([d for d in dscr_series if d is not None])
        if dscr_series
        else 0.0
    )
    ...
    if dscr_min < min_dscr_covenant:
        status = "BREACH"
        violations.append(
            f"Min DSCR {dscr_min:.2f}x < covenant {min_dscr_covenant:.2f}x"
        )
    ...
    logger.info(
        f"Debt planning: Min DSCR={dscr_min:.2f}, Balloon={balloon_payment:,.0f}, Status={status}"
    )
...
if __name__ == "__main__":
    ...
    sample_financing = {
        ...
        "rates": {"lkr_nominal": 0.08, "dfi_nominal": 0.065, "usd_nominal": 0.075},
        "mix": {"lkr_max": 0.45, "dfi_max": 0.10},
    }
    ...
    sample_rows = [
        {"year": i + 1, "cfads_final_lkr": cfads, "fx_rate": fx}
    ...
    print(
        f"  Years 1-3 Debt Service: {[int(ds) for ds in debt_results['debt_service_total'][:3]]}"
    )
