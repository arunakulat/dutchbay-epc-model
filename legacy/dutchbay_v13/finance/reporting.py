...
def generate_covenant_certificate(
    ...
    # Overall status
    all_pass = (
        llcr_covenant_status.get("covenant_status") == "PASS"
        and plcr_covenant_status.get("covenant_status") == "PASS"
        and dscr_min >= dscr_min_covenant
    )
...
def generate_investment_committee_summary(
    ...
    llcr_buffer_pct = (
        ((llcr_min / llcr_min_covenant) - 1) * 100 if llcr_min_covenant else 0
    )
    plcr_buffer_pct = (
        ((plcr_min / plcr_min_covenant) - 1) * 100 if plcr_min_covenant else 0
    )
    dscr_buffer_pct = (
        ((dscr_min / dscr_min_covenant) - 1) * 100 if dscr_min_covenant else 0
    )

    # Overall status
    all_pass = (
        llcr_covenant.get("covenant_status") == "PASS"
        and plcr_covenant.get("covenant_status") == "PASS"
        and dscr_min >= dscr_min_covenant
    )
...
def generate_all_reports(
    ...
    covenant_path = (
        output_dir
        / f"covenant_compliance_certificate_{datetime.now().strftime('%Y%m%d')}.md"
    )
    ic_path = (
        output_dir
        / f"investment_committee_summary_{datetime.now().strftime('%Y%m%d')}.md"
    )
    ...
    generate_investment_committee_summary(metrics_result, irr_result, params, ic_path)

    logger.info(f"✓ All reports generated in {output_dir}")

    return {"covenant_cert": covenant_path, "ic_summary": ic_path}
