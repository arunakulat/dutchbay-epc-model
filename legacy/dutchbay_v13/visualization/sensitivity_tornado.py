...
class TornadoSensitivityAnalyzer:
    ...
    def tornado_chart(
        ...
        for i, (down, up) in enumerate(zip(downside_swings, upside_swings)):
            if down < 0:
                ax.text(
                    down - 0.5, i, f"{down:.2f}%", ha="right", va="center", fontsize=9
                )
            if up > 0:
    ...
    def sensitivity_table(
        self, sensitivities: Dict[str, Dict[str, Any]], metric_type: str = "IRR"
    ) -> pd.DataFrame:
        """
        ...
        for param_name, sens in sorted(
            sensitivities.items(), key=lambda x: x[1]["total_swing"], reverse=True
        ):
            if metric_type == "IRR":
