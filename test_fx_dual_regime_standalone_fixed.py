#!/usr/bin/env python3
"""
DUTCHBAY V13 - P0-2D MODULE TEST (STANDALONE - FIXED)
Version: 1.0.0-production-standalone-fixed
Date: 2025-11-14
Usage: python3 test_fx_dual_regime_standalone_fixed.py
"""
...
# ==================================================================================
# EMBEDDED FX CORRELATION MODULE
# ==================================================================================

class FXCorrelationModule:
    """Multi-Currency FX Correlation & Stress Module"""
    ...
    def __init__(self, monthly_fx_df: pd.DataFrame, config_yaml: Optional[Dict] = None):
        ...
        for old_name, new_name in column_map.items():
            if old_name in self.monthly_fx.columns:
                self.monthly_fx.rename(columns={old_name: new_name}, inplace=True)
        
        # Ensure we have Year-Month as datetime
        ...
        pct_change = self.monthly_fx['Monthly Change (%)'].replace([np.inf, -np.inf], np.nan).dropna()
        self.mean_monthly_change = pct_change.mean()
        self.std_monthly_change = pct_change.std()
        self.max_monthly_move = pct_change.max()
        self.min_monthly_move = pct_change.min()
    ...
    def _generate_shock_scenario(self, base_rate: float, months: int, shock: float) -> np.ndarray:
        ...
        path = np.clip(path, self.min_rate * 0.5, self.max_rate * 1.25)
        return path
    ...
    def assess_revenue_debt_mismatch(self, annual_usd_revenue: float, annual_lkr_debt_service: float,
                                     base_rate: float, scenario_path: np.ndarray) -> Dict:
        """Assess revenue-debt mismatch"""
    ...
    def monte_carlo_var_analysis(self, annual_usd_revenue: float, annual_lkr_debt_service: float,
                                 base_rate: float, num_simulations: int = 1000, confidence_level: float = 0.95) -> Dict:
...
def load_fx_data_dual_regime():
    """Load BOTH recent and historical regimes"""
    ...
    print("\n[HISTORICAL DECADES] Loading for tail-risk...")
    historical_records = []
    ...
    # Convert to DataFrames - use list comprehension to avoid duplicate keys
    recent_df = pd.DataFrame([
        {
            'Year-Month': r['date'],
            'Avg FX Rate': r['avg_rate'],
            'Monthly Change (%)': r.get('monthly_change_pct', 0.0),
            'Rolling 12M Vol (%)': r.get('rolling_12m_vol_pct', 0.0)
        }
        for r in recent_records
    ])
    recent_df['Year-Month'] = pd.to_datetime(recent_df['Year-Month'])
    recent_df = recent_df.sort_values('Year-Month').reset_index(drop=True)
    
    historical_df = pd.DataFrame([
        {
            'Year-Month': h['date'],
            'Avg FX Rate': h['avg_rate'],
            'Monthly Change (%)': h.get('monthly_change_pct', 0.0),
            'Rolling 12M Vol (%)': h.get('rolling_12m_vol_pct', 0.0)
        }
        for h in historical_records
    ])
    historical_df['Year-Month'] = pd.to_datetime(historical_df['Year-Month'])
...
def main():
    ...
    print("\n" + "="*90)
    print("DUTCHBAY V13 - P0-2D MODULE TEST (FIXED)")
    ...
    print(f"\nâœ“ Recent regime data: {len(recent_df)} months")
    print(f"  Date range: {recent_df['Year-Month'].min().strftime('%Y-%m')} to {recent_df['Year-Month'].max().strftime('%Y-%m')}")
    print(f"  FX range: {recent_df['Avg FX Rate'].min():.2f} - {recent_df['Avg FX Rate'].max():.2f} LKR/USD")
    print(f"  Mean rate: {recent_df['Avg FX Rate'].mean():.2f}")
    ...
    print("\n[TEST 1] Recent Regime - Scenario Generation")
    print("-"*90)
    try:
        ...
        for name, path in recent_scenarios.items():
            ...
            status = "âœ“" if result['mean_coverage_ratio'] >= 1.5 else "âœ—"
            print(f"  {name:.<30} {result['mean_coverage_ratio']:>6.2f}x {status}")
        print("âœ“ PASS")
    except Exception as e:
        print(f"âœ— FAIL: {e}")
        import traceback
    ...
    # ========================================================================
    print("\n" + "â–ˆ"*90)
    print("REGIME 2: HISTORICAL TAIL-RISK ANALYSIS")
    print("â–ˆ"*90)
    
    print(f"\nâœ“ Historical regime data: {len(historical_df)} months")
    ...
    print(f"  FX range: {historical_df['Avg FX Rate'].min():.2f} - {historical_df['Avg FX Rate'].max():.2f} LKR/USD")
    ...
    print("\n[TEST 2] Historical Regime - Scenario Generation")
    print("-"*90)
    try:
        historical_scenarios = module_historical.generate_fx_scenarios(305.0, 60)
        historical_results = {}
        print("Coverage Ratios (Historical - Full Period):")
