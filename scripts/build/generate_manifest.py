...
def create_manifest_from_zip(zip_path: Path) -> Dict[str, Any]:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        manifest["metadata"]["total_files"] = len(
            [f for f in file_list if not f.is_dir()]
        )
...
def main(argv: list[str]) -> int:
    ...
    print(
        f"  Size: {manifest['metadata']['total_uncompressed_bytes']:,} → "
        f"{manifest['metadata']['zip_size_bytes']:,} bytes"
    )
    print(
        f"  Compression: {manifest['metadata'].get('compression_ratio_percent', 0):.1f}%"
    )
