...
Usage:
    ...
    python scripts/01_github_repo_scanner.py --output-dir reports/
    python scripts/01_github_repo_scanner.py --include-tests
"""
...
class RepoScanner:
    ...
    def _generate_reports(self):
        ...
        self._generate_markdown_report(output_dir / "migration_status_report.md")
        
        # 2. Repository Structure (JSON)
        self._generate_structure_json(output_dir / "repo_structure.json")
        
        # 3. Dependency Graph (JSON)
        self._generate_dependency_graph(output_dir / "dependency_graph.json")
        
        # 4. Technical Debt Metrics (JSON)
        self._generate_debt_metrics(output_dir / "technical_debt_metrics.json")
...
def main():
    """Main entry point for Dolphin D01 scanner."""
    parser = argparse.ArgumentParser(
        description="Enhanced Repository Scanner for Dolphin Strategy Migration",
    ...
    scanner = RepoScanner(args.repo_root)
    scanner.scan()
    
    print("\n✓ Dolphin D01 execution complete")
    print(f"  Reports available in: {args.repo_root / 'reports'}")
