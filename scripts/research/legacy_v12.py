...
def calculate_npv(rate: float, cash_flows: "list[float] | np.ndarray") -> float:
...
def calculate_irr_robust(
    ...
    if method in ["newton", "both"]:
        try:
            ...
            pass
    # Robust fallback: numpy-financial's IRR. brentq's wide [-0.99, 5.0] bracket
    # overflows on long (20-yr) project cash-flow streams — f(-0.99) divides late
    # flows by 0.01**i and goes non-finite — so project_irr came back None even
    # though a real root exists. npf.irr handles these reliably and is already a
    # core dependency (it is what the v14 finance engine uses).
    try:
        import numpy_financial as npf

        val = float(npf.irr(cf))
        if np.isfinite(val) and -0.99 < val < 5.0:
            npv_check = calculate_npv(val, cf)
            return IRRResult(val, "CONVERGED", "numpy_financial", npv_check, warning)
    except Exception:
        pass
    return IRRResult(
...
def build_financial_model(
    ...
    # These names held per-year scalars inside the amortisation loop above and are
    # now rebound to the full numpy series. mypy flags the scalar->array retype on
    # each line; the reuse is intentional and the model is verified to run, so the
    # [assignment] retype is suppressed (not a real type error).
    usd_prin = np.array(usd_principal_hist)  # type: ignore[assignment]
    usd_int = np.array(usd_interest_hist)  # type: ignore[assignment]
    lkr_prin = np.array(lkr_principal_hist)  # type: ignore[assignment]
    lkr_int = np.array(lkr_interest_hist)  # type: ignore[assignment]
    lkr_ds = np.array(lkr_ds_hist)  # type: ignore[assignment]
