...
def load_fx_data_from_yaml(yaml_file):
    ...
    try:
        ...
        print(
            "  Please ensure fx_complete_50year_data.yaml is in the current directory."
        )
        return None
...
def main():
    ...
    print(
        f"  FX rate range: {fx_df['Avg FX Rate'].min():.2f} - {fx_df['Avg FX Rate'].max():.2f} LKR/USD"
    )
    ...
    try:
        results = {}
        for name, path in scenarios.items():
            result = module.assess_revenue_debt_mismatch(
        ...
        assert all(
            r["mean_coverage_ratio"] > 0 for r in results.values()
        ), "Invalid coverage"

        print("  Coverage Ratios (Debt Service Coverage Ratio):")
        for name, result in sorted(
            results.items(), key=lambda x: x[1]["mean_coverage_ratio"], reverse=True
        ):
            print(
                f"    {name:.<25} Mean: {result['mean_coverage_ratio']:>6.2f}x,  Min: {result['min_coverage_ratio']:>6.2f}x,  Range: {result['min_fx_scenario']:>7.2f}-{result['max_fx_scenario']:>7.2f}"
            )
        ...
        print(
            f"    Status: {'✓ VIABLE' if base_case['mean_coverage_ratio'] >= 1.5 else '✗ AT RISK'}"
        )
    ...
    try:
        ...
        print(
            f"  ✓ Coverage ratio (5th percentile): {mc['percentile_5_coverage']:>6.2f}x"
        )
        print(
            f"  ✓ Coverage ratio (95th percentile): {mc['percentile_95_coverage']:>6.2f}x"
        )
