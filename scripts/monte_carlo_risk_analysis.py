...
cov_matrix = (
    np.diag([cf_std, tariff_std, opex_std, capex_std])
    @ corr_matrix
    @ np.diag([cf_std, tariff_std, opex_std, capex_std])
)
...
for i in tqdm(range(n_scenarios), desc="Scenarios", ncols=80):
    ...
    scenario_params["project"]["capacity_factor"] = max(
        0.1, base_cf * (1 + perturbations[i, 0])
    )
    scenario_params["tariff"]["lkr_per_kwh"] = max(
        5, base_tariff * (1 + perturbations[i, 1])
    )
    scenario_params["opex"]["usd_per_year"] = max(
        1e6, base_opex * (1 + perturbations[i, 2])
    )
    scenario_params["capex"]["usd_total"] = max(
        50e6, base_capex * (1 + perturbations[i, 3])
    )
...
equity_irr_percentiles = analyzer.percentile_analysis(
    equity_irr_arr, [10, 25, 50, 75, 90]
)
...
project_irr_percentiles = analyzer.percentile_analysis(
    project_irr_arr, [10, 25, 50, 75, 90]
)
...
covenant_summary = pd.DataFrame(
    [
        {
            "covenant": f"DSCR < {min_dscr_covenant:.2f}x",
            "breach_probability": f"{dscr_breach_prob*100:.2f}%",
        },
        {
            "covenant": f"LLCR < {min_llcr_covenant:.2f}x",
            "breach_probability": f"{llcr_breach_prob*100:.2f}%",
        },
        {
            "covenant": f"PLCR < {min_plcr_covenant:.2f}x",
            "breach_probability": f"{plcr_breach_prob*100:.2f}%",
        },
    ]
)
...
print(
    f"✓ Equity IRR range: {equity_irr_arr.min()*100:.2f}% to {equity_irr_arr.max()*100:.2f}%"
)
print(
    f"✓ P10/P90 spread: {(equity_irr_percentiles['p90']-equity_irr_percentiles['p10'])*100:.2f}%"
)
print(
    f"✓ Covenant breach risk: {max(dscr_breach_prob, llcr_breach_prob, plcr_breach_prob)*100:.1f}%"
)
