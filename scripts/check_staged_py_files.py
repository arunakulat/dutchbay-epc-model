...
def skip_warnings(output):
    lines = output.split("\n")
    return "\n".join(
        line for line in lines if not any(msg in line for msg in IGNORABLE)
    )
...
def run_cmd(cmd, desc, use_pipenv=False):
    ...
    try:
        logger.info(f">> {desc}")
        proc = subprocess.run(display_cmd, shell=True, capture_output=True, text=True)
...
def main():
    ...
    if not py_files:
        console.print(
            "[yellow]No staged/modified Python files found. Skipping checks.[/yellow]"
        )
        sys.exit(0)
    ...
    console.print(
        Panel.fit(
            "All bulletproofing checks passed on STAGED .py files!", style="green"
        )
    )
