...
def _base_solar_config() -> Dict[str, Any]:
    ...
    cfg = copy.deepcopy(dict(load_scenario_config(str(_REPO / "scenarios" / "dutchbay_basecase_2025Q4.yaml"))))
    cx = SITE["capex_usd"]
    cfg["project"].update(
    ...
    cfg["capex"]["breakdown"] = {
        "epc_usd": cx * 0.875, "development_usd": cx * 0.0625,
        "financing_fees_usd": cx * 0.0375, "contingency_usd": cx * 0.025,
    }
    cfg["opex"]["usd_per_year"] = SITE["opex_usd_yr"]
    cfg["opex"]["breakdown"] = {
        "om_contract_usd": 90_000, "insurance_usd": 20_000,
        "land_lease_usd": 15_000, "admin_usd": 5_000,
    }
    return cfg
...
def run_routines(trials: int = 1000) -> Dict[str, Any]:
    ...
    from analytics.core.sensitivity_runner import run_sensitivity_analysis, _default_parameters
    ...
    pv = compute_solar_aep(SolarResourceConfig(
        latitude=SITE["latitude"], longitude=SITE["longitude"], timezone=SITE["timezone"],
        altitude_m=SITE["altitude_m"], annual_ghi_kwh_m2=SITE["annual_ghi_kwh_m2"],
        dc_capacity_mw=SITE["dc_capacity_mw"], tilt_deg=SITE["tilt_deg"],
        azimuth_deg=SITE["azimuth_deg"], dc_ac_ratio=SITE["dc_ac_ratio"],
        system_loss_pct=SITE["system_loss_pct"]))
    cf = round(pv.capacity_factor * SITE["bifacial_gain"], 4)
    R["resource"] = {
        "cf_mono": pv.capacity_factor, "cf_bifacial": cf,
        "yield_mono": pv.specific_yield_kwh_per_kwp, "poa": pv.poa_kwh_m2,
    ...
    def make(tariff: float, capex: float | None = None) -> Dict[str, Any]:
        ...
        if capex is not None:
            ...
            c["capex"]["breakdown"] = {
                "epc_usd": capex * 0.875, "development_usd": capex * 0.0625,
                "financing_fees_usd": capex * 0.0375, "contingency_usd": capex * 0.025}
        return c
    ...
    def kpis(cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        out = {kk: k.get(kk) for kk in (
            "project_irr", "equity_irr", "project_npv", "min_dscr", "avg_dscr",
            "max_debt_usd", "discount_rate_used")}
    ...
    for t in (fit, 20, comp, 30, 35, 40, 45, 50, 55, 60):
        k = kpis(make(t))
        R["tariff_sweep"].append({"tariff": t, "usc": round(t / (SITE["fx"] / 100), 2),
                                  "projIRR": k["project_irr"], "eqIRR": k["equity_irr"],
                                  "npv": k["project_npv"], "dscr": k["min_dscr"]})
    R["capex_sweep"] = []
    for cxx in (9_000_000, 10_500_000, 12_000_000, 13_200_000, 15_000_000):
        k = kpis(make(comp, capex=cxx))
        R["capex_sweep"].append({"per_kw": cxx / (SITE["dc_capacity_mw"] * 1000),
                                 "projIRR": k["project_irr"], "dscr": k["min_dscr"]})

    # --- 3. Monte Carlo ---
    import numpy as np
    mc_cfg = make(comp)
    mc_cfg["monte_carlo"] = {"parameters": [
        {"name": "project.capacity_factor", "low": 0.165, "high": 0.199, "distribution": "normal"},
        {"name": "capex.usd_total", "low": 10_500_000, "high": 13_500_000, "distribution": "triangular"},
        {"name": "opex.usd_per_year", "low": 110_000, "high": 160_000, "distribution": "uniform"}]}
    ...
    R["mc"] = {
        "n": mc.iterations, "failed": mc.failed_iterations,
        "proj_irr": {q: float(np.percentile(irr, p)) for q, p in (("p10", 10), ("p50", 50), ("p90", 90))} if irr else {},
    ...
    for p, tr in zip(params, suite.tornado_results):
        ...
        torn.append({"param": p.label, "base": tr.base_metric,
                     "low": sh.low_case, "high": sh.high_case,
    ...
    opt = optimize_parameter(str(scen_path), param_path="Financing_Terms.debt_ratio",
                             objective_key="equity_irr", lower=0.30, upper=0.75, direction="max",
                             n_steps=10, constraint_key="min_dscr", constraint_min=1.30)
    R["optimization"] = {
        "curve": [{"x": float(p.value), "obj": float(p.objective),
                   "feasible": bool(p.feasible),
                   "constraint": float(p.constraint) if p.constraint is not None else None}
                  for p in opt.curve],
        "best": ({"x": float(opt.best.value), "obj": float(opt.best.objective)} if opt.best else None)}
    ...
    for td in (1.05, 1.10, 1.20, 1.30, 1.40, 1.50, 1.60):
        ...
        dscr_sweep.append({"target_dscr": td, "gearing": k.get("max_debt_usd", 0) / SITE["capex_usd"],
                           "min_dscr": k["min_dscr"], "projIRR": k["project_irr"],
                           "eqIRR": k["equity_irr"], "npv": k["project_npv"]})
    dopt = optimize_parameter(str(sp), param_path="Financing_Terms.target_dscr",
                              objective_key="equity_irr", lower=1.05, upper=1.60,
                              direction="max", n_steps=12)
    R["dscr"] = {"sweep": dscr_sweep,
                 "opt_best": {"target": float(dopt.best.value), "eqirr": float(dopt.best.objective)} if dopt.best else None}
    return R
...
def make_location_map(charts: Path) -> bool:
        ...
        for i, la in enumerate(lats):
            for j, lo in enumerate(lons):
                ...
                with urllib.request.urlopen(u, timeout=20) as r:
                    d = json.load(r)
                grid[i, j] = d["properties"]["parameter"]["ALLSKY_SFC_SW_DWN"]["ANN"] * 365.0
        bbox = (lons.min() - 0.25, lats.min() - 0.25, lons.max() + 0.25, lats.max() + 0.25)
        gnorth = grid[::-1, :]
    ...
    write_geotiff(np.nan_to_num(gnorth, nan=float(np.nanmean(gnorth))).astype("float32"),
                  bbox, str(charts.parent / "sri_lanka_ghi.tif"),
                  provenance={"source": "NASA POWER ALLSKY_SFC_SW_DWN climatology 2001-2020",
                              "units": "kWh/m2/yr", "generated_by": "DutchBay v14 GeoTIFF export"})
    ...
    ax.annotate(f"PROJECT SITE\n{SITE['location_name']}\n({SITE['latitude']:.3f}N, {SITE['longitude']:.3f}E)",
                ...
                fontweight="bold", color="#1f3a5f", arrowprops=dict(arrowstyle="->", color="#1f3a5f", lw=1))
    for nm, (la, lo) in {"Colombo": (6.93, 79.86), "Jaffna": (9.66, 80.01), "Trincomalee": (8.57, 81.23),
                         "Hambantota": (6.12, 81.12), "Vavuniya": (8.75, 80.50), "Pooneryn": (9.55, 80.23)}.items():
        ax.plot(lo, la, "o", ms=3, color="#444"); ax.text(lo + 0.05, la, nm, fontsize=6.5, color="#222")
    ax.text(80.5, 9.3, "DRY ZONE\n(best solar:\n1,700-1,800 kWh/kWp)", fontsize=6.5, color="#7a3b00", ha="center", fontweight="bold")
    ax.text(79.95, 6.2, "WET ZONE\n(site here)", fontsize=6.5, color="#1f3a5f", ha="center", fontweight="bold")
    cb = fig.colorbar(im, ax=ax, fraction=0.04, pad=0.02); cb.set_label("Annual GHI (kWh/m2/yr)", fontsize=8)
    ax.set_xlabel("Longitude (deg E)", fontsize=8); ax.set_ylabel("Latitude (deg N)", fontsize=8)
...
def make_charts(R: Dict[str, Any], charts: Path) -> None:
    ...
    plt.rcParams.update({"font.size": 10, "font.family": "DejaVu Sans", "axes.grid": True,
                         "grid.alpha": 0.3, "axes.spines.top": False, "axes.spines.right": False, "figure.dpi": 150})
    NAVY, TEAL, AMBER, RED, GREY = "#1f3a5f", "#2a9d8f", "#e9a23b", "#c1432e", "#888"

    def save(fig, name):
        fig.tight_layout(); fig.savefig(f"{charts}/{name}.png", bbox_inches="tight"); plt.close(fig)
    ...
    try:
        ...
        t = pd.date_range("2021-01-01", "2021-12-31 23:00", freq="1h", tz=SITE["timezone"])
        cs = loc.get_clearsky(t, model="ineichen"); sc = SITE["annual_ghi_kwh_m2"] / (cs["ghi"].sum() / 1000)
        ghi = cs["ghi"] * sc; sp = loc.get_solarposition(t)
        ...
        poa = pvlib.irradiance.get_total_irradiance(SITE["tilt_deg"], SITE["azimuth_deg"],
            sp["apparent_zenith"], sp["azimuth"], dni, ghi, dhi, dni_extra=dx, model="haydavies")["poa_global"].clip(lower=0)
        ...
        ac = pvlib.inverter.pvwatts(dc, pdc0=accap / 0.96).clip(upper=accap) * (1 - SITE["system_loss_pct"] / 100) * SITE["bifacial_gain"]
    ...
    ax.set_ylabel("Energy (GWh)"); ax.set_title("Year-1 Monthly Energy Yield - 10 MWp bifacial (P50)", fontweight="bold", color=NAVY)
    ax.text(0.02, 0.92, f"Total {sum(monthly):.1f} GWh/yr | CF {R['resource']['cf_bifacial']:.1%}", transform=ax.transAxes, fontsize=9, color=NAVY)
    save(fig, "01_monthly")
    ...
    ax2.axhline(1.30, color=TEAL, ls=":", lw=1.4); ax2.text(55, 1.33, "DSCR 1.30", color=TEAL, fontsize=8)
    for xv, lab in ((SITE["fit_tariff_lkr"], "FiT"), (SITE["competitive_tariff_lkr"], "USc8"), (50, "b/e ~50")):
        ax.axvline(xv, color=GREY, ls="--", lw=1); ax.text(xv + 0.3, -3.5, lab, rotation=90, fontsize=7, color=GREY, va="bottom")
    ax.set_xlabel("Fixed PPA tariff (LKR/kWh)"); ax.set_ylabel("Project IRR (%)", color=NAVY); ax2.set_ylabel("min DSCR", color=AMBER)
    ax.set_title("Breakeven Frontier - Project IRR & DSCR vs Tariff", fontweight="bold", color=NAVY)
    ax.set_ylim(-6, 10.5); ax2.set_ylim(0.2, 1.5); ax.legend(["Project IRR"], loc="upper left", fontsize=8)
    ...
    p = R["mc"]["proj_irr"]
    for q, lab, col in ((p["p10"] * 100, "P10", "#999"), (p["p50"] * 100, "P50", NAVY), (p["p90"] * 100, "P90", "#999")):
        ax.axvline(q, color=col, ls="--", lw=1.5); ax.text(q, ax.get_ylim()[1] * 0.92, f"{lab}\n{q:.1f}%", ha="center", fontsize=7, color=col)
    ax.axvline(10, color=RED, ls=":", lw=1.6); ax.text(10, ax.get_ylim()[1] * 0.6, "WACC 10%", rotation=90, color=RED, fontsize=8, va="center")
    ax.axvline(0, color="k", lw=0.8); ax.set_xlabel("Project IRR (%)"); ax.set_ylabel("Trials")
    ax.set_title(f"Monte Carlo - Project IRR (n={R['mc']['n']}, LHS) | P(IRR<0)={p['p_below_0'] if False else R['mc']['p_below_0']:.0%}", fontweight="bold", color=NAVY)
    save(fig, "03_mc_irr")

    # 04 tornado
    sd = [d for d in R["sensitivity"] if d["impact"] > 0]; base = R["sensitivity_base"] * 100
    labels = [d["param"] for d in sd][::-1]; lows = [d["low"] * 100 for d in sd][::-1]; highs = [d["high"] * 100 for d in sd][::-1]
    fig, ax = plt.subplots(figsize=(7, 3.4)); y = np.arange(len(labels))
    for i in range(len(labels)):
        lo, hi = sorted([lows[i], highs[i]]); ax.barh(y[i], hi - lo, left=lo, color=TEAL, edgecolor=NAVY, height=0.6)
    ax.axvline(base, color=RED, lw=1.6); ax.text(base, len(labels) - 0.3, f"base {base:.1f}%", color=RED, fontsize=8, ha="center")
    ax.set_yticks(y); ax.set_yticklabels(labels); ax.set_xlabel("Project IRR (%)")
    ax.set_title("One-Way Sensitivity (Tornado) - driver of Project IRR", fontweight="bold", color=NAVY)
    save(fig, "04_tornado")

    # 05 optimisation
    oc = R["optimization"]["curve"]; g = [p["x"] * 100 for p in oc]; eq = [p["obj"] * 100 for p in oc]; feas = [p["feasible"] for p in oc]
    fig, ax = plt.subplots(figsize=(7, 3.6))
    ax.plot(g, eq, "-", color=NAVY, lw=1.5, alpha=0.5)
    ax.scatter([g[i] for i in range(len(g)) if feas[i]], [eq[i] for i in range(len(g)) if feas[i]], color=TEAL, s=55, zorder=5, label="feasible (DSCR>=1.30)")
    ax.scatter([g[i] for i in range(len(g)) if not feas[i]], [eq[i] for i in range(len(g)) if not feas[i]], color=RED, marker="x", s=45, zorder=5, label="infeasible")
    b = R["optimization"]["best"]
    if b:
        ax.scatter([b["x"] * 100], [b["obj"] * 100], edgecolor=AMBER, facecolor="none", s=180, lw=2, zorder=6, label=f"best {b['x']:.0%}")
    ax.axhline(0, color="k", lw=0.8); ax.set_ylim(-30, 5)
    ax.set_xlabel("Gearing (debt ratio, %)"); ax.set_ylabel("Equity IRR (%)")
    ax.set_title("Capital-Structure Optimisation - Equity IRR vs Gearing", fontweight="bold", color=NAVY); ax.legend(loc="lower left", fontsize=8)
    save(fig, "05_optimization")

    # 06 capex
    cs = R["capex_sweep"]; ck = [c["per_kw"] for c in cs]; cpi = [c["projIRR"] * 100 for c in cs]; cd = [c["dscr"] for c in cs]
    fig, ax = plt.subplots(figsize=(7, 3.2)); ax2 = ax.twinx()
    ax.plot(ck, cpi, "-o", color=NAVY, lw=2); ax2.plot(ck, cd, "-s", color=AMBER, lw=1.5)
    ax.axhline(10, color=RED, ls=":", lw=1.2); ax2.axhline(1.30, color=TEAL, ls=":", lw=1.2)
    ax.set_xlabel("Capex (USD/kW)"); ax.set_ylabel("Project IRR (%)", color=NAVY); ax2.set_ylabel("min DSCR", color=AMBER)
    ax.set_title("Capex Sensitivity (at competitive USc 8.0 PPA)", fontweight="bold", color=NAVY)
    save(fig, "06_capex")

    # 07 dscr
    D = R["dscr"]["sweep"]; td = [d["target_dscr"] for d in D]; pi2 = [d["projIRR"] * 100 for d in D]; ei = [d["eqIRR"] * 100 for d in D]; gg = [d["gearing"] * 100 for d in D]
    fig, ax = plt.subplots(figsize=(7, 3.8)); ax2 = ax.twinx()
    ...
    ax.annotate("Project IRR INVARIANT to the DSCR target", xy=(1.30, pi2[3]), xytext=(1.12, 3.2),
                fontsize=7.5, color=NAVY, arrowprops=dict(arrowstyle="->", color=NAVY, lw=0.8))
    ax.set_xlabel("Minimum-DSCR sizing target (x)"); ax.set_ylabel("IRR (%)"); ax2.set_ylabel("Solved gearing (%)", color=AMBER)
    ax.set_ylim(-7, 5); ax2.set_ylim(20, 55)
    ax.set_title("Is the 1.30x DSCR limiting?  Effect of the DSCR target", fontweight="bold", color=NAVY)
...
# PDF RENDERING
# ---------------------------------------------------------------------------
def render_pdf(R: Dict[str, Any], narrative: Dict[str, Any], charts: Path, out_pdf: Path) -> None:
    ...
    S = SITE
    res = R["resource"]; cc = R["case_competitive"]; cf = R["case_fit"]; mc = R["mc"]; D = R["dscr"]
    NAVY = colors.HexColor("#1f3a5f"); TEAL = colors.HexColor("#2a9d8f")
    LIGHT = colors.HexColor("#eef2f6"); GREY = colors.HexColor("#555")
    ...
    body = ParagraphStyle("body", parent=ss["Normal"], fontName="Helvetica", fontSize=9.5, leading=14, alignment=TA_JUSTIFY, spaceAfter=6, textColor=colors.HexColor("#222"))
    h1 = ParagraphStyle("TOCH1", parent=ss["Normal"], fontName="Helvetica-Bold", fontSize=15, leading=18, textColor=NAVY, spaceBefore=14, spaceAfter=8)
    h2 = ParagraphStyle("TOCH2", parent=ss["Normal"], fontName="Helvetica-Bold", fontSize=11.5, leading=15, textColor=TEAL, spaceBefore=10, spaceAfter=4)
    cap = ParagraphStyle("cap", parent=ss["Normal"], fontName="Helvetica-Oblique", fontSize=8, leading=11, textColor=GREY, alignment=1, spaceAfter=10)
    bullet = ParagraphStyle("bullet", parent=body, leftIndent=14, spaceAfter=3)
    cH = ParagraphStyle("cH", parent=ss["Normal"], fontName="Helvetica-Bold", fontSize=8.3, leading=10, textColor=colors.white, alignment=1)
    cHL = ParagraphStyle("cHL", parent=cH, alignment=TA_LEFT)
    cB = ParagraphStyle("cB", parent=ss["Normal"], fontName="Helvetica", fontSize=8.3, leading=10.5, textColor=colors.HexColor("#222"), alignment=1)
    cBL = ParagraphStyle("cBL", parent=cB, alignment=TA_LEFT)
    mK = ParagraphStyle("mK", parent=ss["Normal"], fontName="Helvetica-Bold", fontSize=9.5, leading=13, textColor=NAVY)
    mV = ParagraphStyle("mV", parent=ss["Normal"], fontName="Helvetica", fontSize=9.5, leading=13, textColor=colors.HexColor("#222"))
    ...
    def datatable(header, rows, cw, hi=()):
        ...
        data += [[Paragraph(str(r[0]), cBL)] + [Paragraph(str(c), cB) for c in r[1:]] for r in rows]
    ...
    # ---- page templates ----
    def hf(canvas, doc):
        canvas.saveState(); canvas.setFillColor(NAVY); canvas.rect(0, A4[1] - 1.05 * cm, A4[0], 1.05 * cm, fill=1, stroke=0)
        canvas.setFillColor(colors.white); canvas.setFont("Helvetica-Bold", 7.5)
        canvas.drawString(1.6 * cm, A4[1] - 0.68 * cm, "DUTCHBAY EPC MODEL v14  -  TECHNICAL & FINANCIAL ASSESSMENT")
        canvas.drawRightString(A4[0] - 1.6 * cm, A4[1] - 0.68 * cm, f"10 MW SOLAR PV  -  {S['location_name'].upper()}")
        canvas.setFillColor(GREY); canvas.setFont("Helvetica", 7.5)
        canvas.drawString(1.6 * cm, 0.7 * cm, "Generated by the DutchBay v14 project-finance engine - sample output. Model-derived; see assumptions & sources.")
        canvas.drawRightString(A4[0] - 1.6 * cm, 0.7 * cm, f"Page {doc.page}")
        canvas.setStrokeColor(colors.HexColor("#ccc")); canvas.line(1.6 * cm, 1.0 * cm, A4[0] - 1.6 * cm, 1.0 * cm); canvas.restoreState()
    ...
    doc = Doc(str(out_pdf), pagesize=A4, leftMargin=1.6 * cm, rightMargin=1.6 * cm, topMargin=1.4 * cm, bottomMargin=1.2 * cm,
              title=f"10 MW Solar PV Assessment - {S['location_name']}", author="DutchBay v14")
    doc.addPageTemplates([PageTemplate(id="title", frames=[tfr], onPage=title_bg),
                          PageTemplate(id="main", frames=[fr], onPage=hf)])

    # ---- COVER ----
    story.append(Spacer(1, 1.2 * cm))
    story.append(Paragraph("Independent Technical &amp;<br/>Financial Assessment",
                           ParagraphStyle("t", fontName="Helvetica-Bold", fontSize=27, leading=31, textColor=colors.white)))
    story.append(Spacer(1, 0.25 * cm))
    story.append(Paragraph("10&nbsp;MW Single-Technology Solar PV (Bifacial)",
                           ParagraphStyle("s1", fontName="Helvetica", fontSize=15, leading=19, textColor=colors.HexColor("#d9e6f2"))))
    story.append(Paragraph(S["location_name"],
                           ParagraphStyle("s2", fontName="Helvetica-Bold", fontSize=18, leading=22, textColor=colors.white)))
    story.append(Paragraph(f"{S['location_detail']} &mdash; {S['latitude']:.4f}&deg;N, {S['longitude']:.4f}&deg;E",
                           ParagraphStyle("s3", fontName="Helvetica", fontSize=10.5, leading=14, textColor=colors.HexColor("#d9e6f2"))))
    story.append(Spacer(1, 4.6 * cm))
    meta = [["Prepared with", "DutchBay EPC Model &mdash; v14 project-finance engine"],
            ["Routines", "Resource (pvlib) &middot; finance &middot; Monte&nbsp;Carlo &middot; sensitivity &middot; optimisation &middot; GeoTIFF map"],
    ...
    mt = Table([[Paragraph(a, mK), Paragraph(b, mV)] for a, b in meta], colWidths=[3.6 * cm, 11.9 * cm])
    mt.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP"), ("TOPPADDING", (0, 0), (-1, -1), 5),
                            ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("LINEBELOW", (0, 0), (-1, -1), 0.4, colors.HexColor("#ccc"))]))
    story.append(mt)
    ...
    H1("Contents")
    toc = TableOfContents()
    toc.levelStyles = [ParagraphStyle("toc1", fontName="Helvetica-Bold", fontSize=10, leading=16, textColor=NAVY),
                       ParagraphStyle("toc2", fontName="Helvetica", fontSize=9, leading=13, leftIndent=14, textColor=colors.HexColor("#333"))]
    story.append(toc)
    story.append(Spacer(1, 8))
    H2("List of Figures")
    figs = ["Sri Lanka solar resource (GHI) and project site", "Year-1 monthly energy yield",
            "Breakeven frontier - project IRR & DSCR vs tariff", "Capex sensitivity",
            "Monte Carlo distribution of project IRR", "One-way sensitivity (tornado)",
            "Capital-structure optimisation (equity IRR vs gearing)", "Effect of the DSCR sizing target"]
    ...
    P(f"This report is an independent technical and financial assessment of a proposed <b>10&nbsp;MW single-technology solar "
    ...
    P(f"<b>Two locational facts frame the assessment.</b> First, the coordinate falls beside the national Parliament and the "
    ...
    P(f"<b>Headline finding: the project is not viable at any tariff currently available to a standalone 10&nbsp;MW plant.</b> "
      ...
      f"{pc(1 - mc['p_below_0'])}.")
    P("The breakeven analysis shows the plant needs ~<b>LKR&nbsp;50/kWh</b> for a 1.30&times; DSCR and ~<b>LKR&nbsp;60/kWh</b> "
    ...
    datatable(["Resource metric", "Value", "Source"],
              [["Annual GHI", f"{S['annual_ghi_kwh_m2']:,.0f} kWh/m2/yr", "Solargis v2.2.68 (bankable)"],
               ["GHI cross-check", f"~{S['nasa_ghi']:,.0f} kWh/m2/yr", "NASA POWER 2001-2020"],
               ["PVOUT (monofacial)", f"{S['solargis_pvout']:,.0f} kWh/kWp", "Solargis"],
               ["DutchBay model yield", f"{res['yield_mono']:,.0f} kWh/kWp", "pvlib producer (within 2%)"],
    ...
    render_section("tech")
    datatable(["Rank / Module", "Cell", "Power", "Bifaciality", "Temp coeff", "Note"],
              [["1 - Trina Vertex N", "n-TOPCon", "720 W", "80+-5%", "-0.29%/C", "PRIMARY: salt-mist + ammonia certified; bankability"],
               ["2 - Risen Hyper-ion", "n-HJT", "700-725 W", "~85-90%", "-0.24%/C", "Best hot-climate physics; HJT premium"],
               ["3 - Huasun Himalaya G12", "n-HJT", "700-720 W", "85+-5%", "-0.26%/C", "HJT yield + India-BIS channel"],
               ["4 - Astronergy N7 Pro", "n-TOPCon", "670 W", "85+-5%", "-0.26%/C", "Highest efficiency (24.8%); newest"]],
              [4.3 * cm, 1.7 * cm, 1.9 * cm, 1.9 * cm, 1.8 * cm, 3.9 * cm])

    render_section("yield-method")
    FIG("01_monthly"); CAP("Modelled year-1 monthly energy yield (P50). Equatorial sites show low seasonality.")

    render_section("finance-method")

    render_section("results")
    datatable(["Metric", "Base / Regulatory (FiT 14.46)", "Investment (USc 8.0 / 26.70)"],
              [["Project IRR (unlevered)", pc(cf["project_irr"]), pc(cc["project_irr"])],
               ["Project NPV @ WACC", usd(cf["project_npv"]), usd(cc["project_npv"])],
               ["Minimum DSCR (70% gearing)", f"{cf['min_dscr']:.2f}", f"{cc['min_dscr']:.2f}"],
               ["WACC / discount rate", pc(cf["discount_rate_used"]), pc(cc["discount_rate_used"])],
               ["Verdict", "Not viable", "Not viable (sub-WACC)"]],
              [5.5 * cm, 5.0 * cm, 5.0 * cm], hi=(5,))
    FIG("02_tariff_breakeven"); CAP("Breakeven frontier. Project IRR (left) and minimum DSCR (right) vs fixed PPA tariff.")
    FIG("06_capex"); CAP("Capex sensitivity at the competitive USc 8.0 PPA.")

    render_section("risk")
    FIG("03_mc_irr"); CAP(f"Monte Carlo distribution of project IRR ({mc['n']:,} LHS trials); the whole distribution lies far below the ~10% WACC.")
    FIG("04_tornado"); CAP("Tornado: impact of each driver on project IRR. Capacity factor and tariff dominate.")
    FIG("05_optimization"); CAP("Equity IRR vs gearing under the 1.30x DSCR constraint; feasible only below ~35% gearing.")

    render_section("dscr")
    datatable(["Min-DSCR target", "Solved gearing", "Project IRR", "Equity IRR"],
              [[f"{d['target_dscr']:.2f}x", f"{d['gearing']*100:.0f}%", pc(d["projIRR"]), pc(d["eqIRR"])] for d in D["sweep"]],
              [4.0 * cm, 4.0 * cm, 3.7 * cm, 3.8 * cm])
    FIG("07_dscr"); CAP("Effect of the DSCR sizing target. Project IRR is flat (financing cannot change the asset return); equity IRR improves as the structure deleverages - the signature of negative carry.")
    ...
    datatable(["Parameter", "Value", "Basis"],
              [["DC nameplate", "10 MWp (10 MWac, 1.2 ILR)", "Brief"],
               ["Net capacity factor (P50)", f"{pc(res['cf_bifacial'])} (mono {pc(res['cf_mono'])} +7% bifacial)", "pvlib on Solargis GHI"],
               ["Annual GHI", f"{S['annual_ghi_kwh_m2']:,.0f} kWh/m2/yr", "Global Solar Atlas / Solargis"],
               ["Tilt / azimuth / DC-AC", f"{S['tilt_deg']:.0f}deg / {S['azimuth_deg']:.0f}deg / {S['dc_ac_ratio']}", "Solargis optimum / standard"],
               ["System losses / degradation", f"{S['system_loss_pct']:.0f}% / {S['degradation_pct']}%/yr", "Industry standard"],
               ["Project life / PPA tenor", f"{S['life_years']} yr / {S['ppa_term_years']} yr", "Solar asset / PPA"],
               ["Capex (base)", f"{usd(S['capex_usd'])} (${S['capex_usd']/S['dc_capacity_mw']/1000:,.0f}/kW)", "IEA + Siyambalanduwa range"],
               ["Opex", f"${S['opex_usd_yr']/1000:.0f}k/yr ($13/kW-yr)", "Industry standard"],
               ["FiT tariff", f"LKR {S['fit_tariff_lkr']}/kWh (>1 MW)", "SLSEA June-2025 FiT"],
               ["Competitive tariff", f"USc {S['competitive_usc']} = LKR {S['competitive_tariff_lkr']}/kWh fixed", "Siyambalanduwa PPA"],
               ["Exchange rate", f"LKR {S['fx']}/USD, 3%/yr depr.", "DutchBay canonical FX"],
               ["Gearing / tenor / min-DSCR", "70% (sized) / 15 yr / 1.30x", "DutchBay base financing"],
               ["WACC / cost of equity", f"{pc(cc['discount_rate_used'])} / ~12%", "Build-up, rupee environment"]],
              [4.6 * cm, 5.4 * cm, 5.5 * cm])

    H1("Appendix B &mdash; Glossary")
    for term, d in [("GHI", "Global Horizontal Irradiation - total solar energy on a horizontal surface (kWh/m2/yr)."),
                    ("PVOUT", "Specific PV power output - annual energy per installed kWp (kWh/kWp/yr)."),
                    ("Capacity factor (CF)", "Average output as a fraction of nameplate; energy / (capacity x 8,760 h)."),
                    ("ILR / DC-AC ratio", "Inverter loading ratio - installed DC capacity / AC nameplate."),
                    ("CFADS", "Cash Flow Available for Debt Service - operating cashflow before financing."),
                    ("DSCR", "Debt-Service Cover Ratio - CFADS / scheduled debt service; the covenant is the minimum allowed."),
                    ("Project / Equity IRR", "Unlevered (whole-asset) vs levered (shareholder) internal rate of return."),
                    ("WACC", "Weighted Average Cost of Capital - the blended required return used to discount the project."),
                    ("Negative carry", "When the project return is below the cost of debt, so leverage reduces equity return."),
                    ("Bifaciality", "The fraction of front-side efficiency a module's rear side achieves.")]:
        B(f"<b>{term}:</b> {d}")

    H1("Appendix C &mdash; Data Sources & References")
    for r_ in ["Global Solar Atlas / Solargis (v2.2.68) - site GHI 1,909 and PVOUT 1,524 kWh/kWp.",
...
def main() -> None:
    global _OUT_PATH
    ap = argparse.ArgumentParser(description="Generate the solar PV assessment PDF report.")
    ap.add_argument("--out", default=str(Path.home() / "Downloads" / "solar_assessment_report"))
    ap.add_argument("--trials", type=int, default=1000, help="Monte Carlo trials")
    args = ap.parse_args()

    out = Path(args.out); charts = out / "charts"
