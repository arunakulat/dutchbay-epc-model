...
def add_debt_service_detail_sheet(
    wb: openpyxl.Workbook, results: dict[str, Any]
) -> None:
    ...
    for row_data in annual_rows:
        ...
        status = (
            "Interest-Only"
            if principal == 0
            else ("Amortizing" if dscr > 0 else "Grace")
        )
...
def add_kpi_summary_sheet(wb: openpyxl.Workbook, results: dict[str, Any]) -> None:
    ...
    row = 4
    total_interest = sum(
        float(r.get("interest_usd", 0)) for r in results.get("annual_rows", [])
    )
...
def export_to_excel(config_path: str, output_path: str) -> None:
    ...
    results = run_v14_pipeline(
        config_path, validation_mode="off", allow_fx_degradation=True
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("\nExample:")
        print(
            "  python export_to_excel.py scenarios/dutchbay_lendercase_2025Q4.yaml out/analysis.xlsx"
        )
        sys.exit(1)
