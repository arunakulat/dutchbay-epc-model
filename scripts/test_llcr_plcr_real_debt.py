...
assert (
    "debt_outstanding" in debt_results and len(debt_results["debt_outstanding"]) > 0
), "Debt outstanding not found!"
...
assert all(
    x >= 0 for x in llcr_result["llcr_series"]
), "LLCR calculation negative value!"
assert all(
    x >= 0 for x in plcr_result["plcr_series"]
), "PLCR calculation negative value!"
assert (
    0.95 < min(plcr_result["plcr_series"]) / min(llcr_result["llcr_series"]) <= 1.5
), "PLCR should always be >= LLCR but not wildly greater"
...
assert (
    metrics_summary["llcr"]["llcr_min"] == llcr_result["llcr_min"]
), "metrics.py LLCR does not match direct calculation."
assert (
    metrics_summary["plcr"]["plcr_min"] == plcr_result["plcr_min"]
), "metrics.py PLCR does not match direct calculation."
