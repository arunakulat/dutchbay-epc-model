...
def main():
    ...
    print(
        f"  FX range: {recent_df['avg_rate'].min():.2f} - {recent_df['avg_rate'].max():.2f} LKR/USD"
    )
    ...
    try:
        module_recent = FXCorrelationModule(
            recent_df.rename(
                columns={
    ...
    try:
        ...
        for name, path in recent_scenarios.items():
            ...
            status = (
                "âœ“ VIABLE" if result["mean_coverage_ratio"] >= 1.5 else "âœ— RISK"
            )
    ...
    print(
        f"  FX range: {historical_df['avg_rate'].min():.2f} - {historical_df['avg_rate'].max():.2f} LKR/USD"
    )
    ...
    try:
        module_historical = FXCorrelationModule(
    ...
    print(f"  {'-'*70}")
    print(
        f"  {'Mean FX Rate':<30} {module_recent.mean_rate:>13.2f} {module_historical.mean_rate:>13.2f} {module_historical.mean_rate - module_recent.mean_rate:>13.2f}"
    )
    print(
        f"  {'Std Deviation':<30} {module_recent.std_rate:>13.4f} {module_historical.std_rate:>13.4f} {module_historical.std_rate - module_recent.std_rate:>13.4f}"
    )
    print(
        f"  {'Autocorr (lag-1)':<30} {module_recent.autocorr_lag1:>13.4f} {module_historical.autocorr_lag1:>13.4f} {module_historical.autocorr_lag1 - module_recent.autocorr_lag1:>13.4f}"
    )
    print(
        f"  {'Max Rate':<30} {module_recent.max_rate:>13.2f} {module_historical.max_rate:>13.2f} {module_historical.max_rate - module_recent.max_rate:>13.2f}"
    )
    ...
    print(
        f"  Interpretation: Historical scenarios show {abs(historical_base - recent_base):.1%} {'lower' if historical_base < recent_base else 'higher'} coverage"
    )
    ...
    print(
        f"  - Suggested assumption: {min(recent_base, historical_base):.2f}x (conservative)"
    )
