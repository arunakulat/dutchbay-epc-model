...
from dutchbay_v13.visualization.sensitivity_tornado import \
    TornadoSensitivityAnalyzer
...
for spec in sensitivity_specs:
    ...
    try:
        sens_result = analyzer.calculate_sensitivity(
            param_name, param_path, low_pct, high_pct, calculate_all_returns, metric
        )
...
sorted_sens = sorted(
    equity_irr_sens.items(), key=lambda x: x[1]["total_swing"], reverse=True
)
