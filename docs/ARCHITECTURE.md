...
## Package import graph (generated)

The following diagram is **derived from the actual import graph**, not drawn by hand:
`scripts/gen_architecture_diagram.py` walks every first-party source package, parses
each module with `ast`, and draws an edge `A --> B` whenever a module in package `A`
imports package `B`. Regenerate it with `python scripts/gen_architecture_diagram.py`;
`--check` fails CI if it drifts (`tests/docs/test_architecture_diagram.py`), so the
picture cannot fall out of sync with the code the way a hand-maintained diagram would.

Assumptions and limitations: nodes are top-level importable packages only (module- and
function-level structure is out of scope); relative (intra-package) imports are not
drawn; tooling, tests, examples, and vendored directories are excluded. The canonical
artifact is [`architecture_import_graph.mmd`](architecture_import_graph.mmd); the block
below mirrors it. A cycle in the graph (e.g. `analytics` and `finance` importing each
other at package granularity) reflects call-site lazy imports, not an import-time cycle.

<!-- BEGIN-GENERATED-IMPORT-GRAPH -->
```mermaid
...
flowchart LR
    ...
    app --> wind_resource
    finance --> analytics
    solar_resource --> analytics
    solar_resource --> finance
    wind_resource --> analytics
```
