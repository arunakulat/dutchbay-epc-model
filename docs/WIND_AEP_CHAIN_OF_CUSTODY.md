...
> **TA caveat — ERA5 is reanalysis, not measurement.** ERA5 is a modelled atmospheric reanalysis at ~31 km grid resolution, not on-site met-mast data. There is no on-site measurement campaign anywhere in the committed evidence chain: the lender scenario's `wind_resource.data_confidence` records a screening-grade / pre-measurement basis (`scenarios/dutchbay_lendercase_2025Q4.yaml`), consistent with the limitation of record in `config/report_defaults.yaml` ('Wind resource measurement basis (screening-grade)'). An earlier revision of that field asserted validation against a one-year on-site met-mast campaign; no such campaign exists and the label was retracted under #961 — it must not be cited as corroboration. For bankability, ERA5 should be MCP-correlated (Measure-Correlate-Predict) against a site mast; `wind_resource/mcp.py` implements MCP but no production code path imports or calls it, so the financed chain extrapolates ERA5 directly.
...
| Source label | "ECMWF ERA5 Reanalysis 2020-2024", screening-grade / pre-measurement (no on-site mast; MCP unwired — #961) | "OEM_ENVISION_EN171_10_PC (extrapolated)" |
...
| R5 | Resource is **ERA5 reanalysis** (~31 km) with **no on-site measurement campaign**; the MCP module exists but is **unwired** — no production code path imports or calls it | `era5_fetcher.py:456`; scenario `wind_resource.data_confidence` (screening-grade / pre-measurement, #961); `wind_resource/mcp.py` (`run_mcp` / `MCPResult` / `mcp_settings`) present with no production importer | High — no bankable resource basis until an on-site campaign (>=12 months, met mast or IEC-accepted LiDAR/SoDAR) is long-term-adjusted through MCP. Regraded from Medium under #961: the former grade was calibrated against a met-mast campaign that does not exist, and `config/report_defaults.yaml` already grades this same Resource risk `severity: "high"` |
