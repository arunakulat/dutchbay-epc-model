"""
Returns Module for DutchBay V13 - Option A Step 4 (Updated with debt_outstanding tracking)
Full tax, regulatory, risk, and DSCR-sculpted logic with MC FX support.
Now includes debt_outstanding tracking for LLCR/PLCR reporting.
Audit-grade, drop-in file.

Author: DutchBay V13 Team, Nov 2025
Version: 2.1 (With debt_outstanding tracking for LLCR/PLCR)
"""
...
def corrected_project_cashflows_and_ds(params: dict, montecarlo_fx: Optional[List[float]] = None):
    ...
    Returns: (project_cfs, equity_cfs, ds_list, cfads_list, tax_list, cfads_gross, debt_outstanding_list)
    
    Now tracks debt outstanding for each year for LLCR/PLCR calculations.
    """
    ...
    # Regulatory/grid loss from YAML
    ...
    debt_outstanding_list = []  # ← NEW: Track debt outstanding each year
    ...
    for year in range(project_life):
        ...
        # Store debt outstanding at start of year (before repayment)
        debt_outstanding_list.append(debt_left)
        
        # Debt service sculpted to post-tax cashflow
        ...
        # Tax (corporate, depreciation shield)
    ...
    return project_cfs, equity_cfs, ds_list, cfads_list, ctax_list, cfads_gross, debt_outstanding_list
...
def irr(cashflows: List[float], guess: float = 0.10) -> Optional[float]:
    """Internal rate of return (using numpy-financial if available, else fallback)."""
    try:
...
def calculate_all_returns(params: dict, montecarlo_fx: Optional[List[float]] = None) -> Dict[str, Any]:
    """
    Calculate all returns and metrics.
    
    Returns comprehensive dict with project/equity IRR/NPV, cashflows, debt service, 
    taxes, and debt outstanding for coverage ratio calculations.
    """
    project_cfs, equity_cfs, ds_list, cfads_list, tax_list, cfads_gross, debt_outstanding_list = \
        corrected_project_cashflows_and_ds(params, montecarlo_fx)
    ...
    return {
        ...
        'equity': {
            ...
            'equity_cfs': equity_cfs,
            'equity_investment': -equity_cfs[0],
        },
        'debt_service': ds_list,
        'debt_outstanding': debt_outstanding_list,  # ← NEW: Now available for LLCR/PLCR
        'cfads': cfads_list,
        'tax': tax_list,
